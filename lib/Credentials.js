...
class Credentials {
  ...
  // 1. Explicilty set via Advertising class constructor
  // 2. Credentials file
  ...
  load(){
  	...
  	if (!loaded_credentials){
			try {
				let credentials_file = this._path ? this._path : this._getHomeDir() + '/.amzadapi/credentials';
		...
		return {
			id:loaded_credentials['AMAZON_AD_CLIENT_ID'],
			secret:loaded_credentials['AMAZON_AD_CLIENT_SECRET']
		};
	}
