...
class Advertising {
  ...
  constructor(config){
    ...
    this._big_int_keys = ['campaignId', 'keywordId', 'adGroupId', 'targetId', 'profileId', 'portfolioId'];
    this._param_regex = {
      '{profileId}':'[0-9]*',
      '{campaignId}':'[0-9]*',
      '{adGroupId}':'[0-9]*',
  ...
  _parseResult(res){
    try {
      // Fix for cases where body has no unauthorized message although it has statusCode 401
      if (res.body === '' && res.statusCode === 401 && res.headers['www-authenticate'].includes('authorized customer is null')){
        return {
          code:'UNAUTHORIZED',
          details:'HTTP 401 Unauthorized'
        };
      }
      return JSONbig.parse(res.body);
    } catch (e){
      throw new CustomError({
