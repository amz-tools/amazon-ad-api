...
class Advertising {
  ...
  _createRequest(req_params) {
    ...
    if (req_params.headers)
      req.headers = Object.assign(req.headers, req_params.headers);
    return req;
  }
  ...
  async _wait(tries) {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        resolve();
      }, tries * this._options.retry_after);
    });
  }
  ...
  async _getReportV3(req_params) {
    let res;
    if (!req_params.reportId) {
      let configuration = {
        adProduct:
          req_params.campaignType === "sponsoredBrands"
            ? "SPONSORED_BRANDS"
            : req_params.campaignType === "sponsoredDisplay"
            ? "SPONSORED_DISPLAY"
            : "SPONSORED_PRODUCTS",
        timeUnit: "DAILY",
        format: "GZIP_JSON",
      ...
      if (req_params.recordType === "campaigns") {
        configuration.reportTypeId =
          req_params.campaignType === "sponsoredBrands"
            ? "sbCampaigns"
            : req_params.campaignType === "sponsoredDisplay"
            ? "sdCampaigns"
        ...
        req_params.body?.segment === "placement"
          ? configuration.groupBy.push("campaignPlacement")
          : configuration.groupBy.push("campaign");
      } else if (req_params.recordType === "adGroups") {
        // Adgroups of sponsored product campaigns are requested via campaigns report, so "spCampaigns" as reportTypeId is correct
        configuration.reportTypeId =
          req_params.campaignType === "sponsoredBrands"
            ? "sbAdGroup"
            : req_params.campaignType === "sponsoredDisplay"
        ...
        } else if (req_params.body?.segment === "matchedTarget") {
          configuration.reportTypeId = "sdTargeting";
          configuration.groupBy.push("matchedTarget");
        } else {
          configuration.reportTypeId =
            req_params.campaignType === "sponsoredBrands"
              ? "sbTargeting"
        ...
        configuration.reportTypeId =
          req_params.campaignType === "sponsoredBrands"
            ...
            : "spPurchasedProduct";
        req_params.campaignType === "sponsoredBrands"
          ? configuration.groupBy.push("purchasedAsin")
          : configuration.groupBy.push("asin");
      }
      if (req_params.recordType === "keywords") {
      ...
      res = await this.callAPI(req_params);
    } else {
      res = {
        reportId: req_params.reportId
      };
    }
    if (res.reportId) {
