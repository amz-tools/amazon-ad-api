...
class Advertising {
  ...
  async _getReportURL(report_id, tries = 0){
  	let res = await this.callAPI({
			method:'GET',
			operation:'/v2/reports/' + report_id
		});
		if (res.status === 'SUCCESS'){
			return res.location;
		}
		tries++;
		if (tries > this._options.max_retries){
			throw new CustomError({
		...
		await this._wait();
		return await this._getReportURL(report_id, tries);
  }
  ...
  _validateReportParams(req_params){
  	...
  	if (!req_params.recordType){
  		throw new CustomError({
        code:'NO_RECORD_TYPE_PROVIDED',
        message:'Please provide a recordType'
      });
  	}
  	if (!reports[req_params.campaignType][req_params.recordType]){
  		throw new CustomError({
  ...
  async getReport(req_params){
  	...
  	req_params.operation = reports[req_params.campaignType][req_params.recordType].operation;
  	// Set default metrics (all available metrics) if no metrics given
		req_params.body.metrics = req_params.body.metrics || this._addDefaultMetrics(req_params);
  	let res = await this.callAPI(req_params);
  	if (res.reportId){
  		let report_url = await this._getReportURL(res.reportId);
