...
class Advertising {
  ...
  _map = {
    ...
    sponsoredBrands: (req_params) => {
      ...
      if (req_params.recordType === "campaigns") {
        if (req_params.body?.segment === "placement") {
          configuration.reportTypeId = "sbCampaignPlacement";
          configuration.groupBy.push("campaignPlacement");
        } else {
          configuration.reportTypeId = "sbCampaigns";
          configuration.groupBy.push("campaign");
        }
      } else if (req_params.recordType == "adGroups") {
        configuration.reportTypeId = "sbAdGroup";
        configuration.groupBy.push("adGroup");
      }
      // } else if (req_params.recordType === "productAds") {
      //   configuration.reportTypeId = "spAdvertisedProduct";
  ...
  async _getReportV3(req_params) {
    let configuration = this._map[req_params.campaignType](req_params);
    ...
    // if (req_params.body?.creativeType) {
    //   configuration.filters = [
    //     {
    //       field: "creativeType",
    //       values: [req_params.body.creativeType]
    //     }
    //   ];
    // }
  ...
  async callAPI(req_params, tries = 0) {
    ...
    console.dir(req_params, { depth: null });
